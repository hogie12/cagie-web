  ...
  return (
    <CoupleDataProvider>
      <div className="h-[100dvh] w-full bg-background text-foreground overflow-hidden flex flex-col">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 h-[76px] bg-gradient-to-r from-white/90 via-white/80 to-nav-bg/80 backdrop-blur-2xl rounded-t-[2rem] flex items-center justify-around px-2 z-50 shadow-[0_-8px_32px_rgba(0,0,0,0.08)] border-t border-white/60">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {isProfile ? (
                  <div
                    className={`w-7 h-7 rounded-full overflow-hidden flex items-center justify-center mb-0.5 ${isActive ? "ring-2 ring-nav-inactive" : ""}`}
                  >
                    {userPhotoURL ? (
                      <img
                        src={userPhotoURL}
        ...
        <div className="hidden md:flex fixed left-0 top-0 bottom-0 w-20 bg-card border-r border-border flex-col items-center py-8 space-y-8 z-50">
          <Link
            href="/profile"
            className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-primary-foreground font-bold shadow-md overflow-hidden hover:opacity-90 transition-opacity"
          >
            {userPhotoURL ? (
              <img
                src={userPhotoURL}
