        ...
        <AnimatePresence>
          {notes.map((note) => {
            ...
            const safeX = Math.max(
              10,
              Math.min(note.x, windowSize.width - 190),
            );
            const safeY = Math.max(
              80,
              Math.min(note.y, windowSize.height - 250),
            );
            ...
            return (
              ...
              >
                <div
                  className={`flex-1 text-sm font-medium whitespace-pre-wrap ${colorTheme.text} leading-relaxed`}
                >
                  {note.text}
                </div>
                ...
                <div className="mt-4 flex justify-between items-end opacity-50 group-hover:opacity-100 transition-opacity">
                  <div className={`text-[10px] font-bold ${colorTheme.text}`}>
                    {isMine ? userName || "Me" : partnerName || "Partner"}
                  </div>
                ...
                <div
                  className={`absolute bottom-0 left-0 w-6 h-6 bg-black/10 rounded-tr-xl rounded-bl-3xl pointer-events-none`}
                />
              </motion.div>
            );
          })}
                ...
                <div>
                  ...
                  <div className="flex gap-3 justify-between">
                    {COLORS.map((c) => (
                      ...
                      >
                        {selectedColor === c.id && (
                          <Check size={18} className={c.text} />
                        )}
                      </button>
                    ))}
                  </div>
