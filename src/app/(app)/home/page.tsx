...
import { Heart, Upload, Calendar as CalendarIcon, StickyNote as StickyNoteIcon, Camera, Send } from "lucide-react";
...
export default function HomePage() {
  ...
  const partnerGreeting = dashboard?.greetingBy !== user?.uid ? dashboard?.greeting : null;
  ...
  const upcomingEvents = events
    ...
    .sort((a, b) => {
      if (a.dateStr === b.dateStr) return a.startTime.localeCompare(b.startTime);
    ...
    <div className="h-full overflow-y-auto p-6 md:p-10 pb-[100px] md:pb-10 bg-gradient-to-br from-background to-rose-50/30">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          <h1 className="text-3xl font-bold">Good morning, {user?.displayName?.split(" ")[0]}! ✨</h1>
          ...
          {partnerGreeting ? (
            <div className="p-6 bg-pink-100/50 rounded-3xl border border-pink-200 shadow-sm relative overflow-hidden">
              ...
              <p className="text-xl font-medium text-pink-900 relative z-10">&quot;{partnerGreeting}&quot;</p>
              <p className="text-sm text-pink-700 mt-2 relative z-10 font-medium">— From your partner 💖</p>
            </div>
          ) : (
            <div className="p-5 bg-white rounded-3xl border border-slate-100 shadow-sm">
              <p className="text-sm text-slate-500 mb-3">Leave a cute greeting for your partner to wake up to!</p>
              ...
              {dashboard?.greetingBy === user?.uid && (
                <p className="text-xs text-green-600 mt-3 font-medium">✓ You left a greeting. Waiting for them to see it!</p>
              )}
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-4">
            {dashboard?.papUrl ? (
              <div className="relative aspect-square md:aspect-video rounded-3xl overflow-hidden group">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                  <p className="text-white font-medium">
                    {isPapFromPartner ? "Uploaded by your partner 📸" : "Uploaded by you 📸"}
                  </p>
                </div>
              ...
              >
                ...
                <p className="text-sm text-slate-400 mt-1">Tap to snap or upload a photo</p>
              </div>
            )}
            ...
            <div className="flex-1 space-y-3">
              {upcomingEvents.length > 0 ? (
                upcomingEvents.map(event => {
                  ...
                  return (
                    <div key={event.id} className="flex gap-4 items-center p-3 rounded-2xl hover:bg-slate-50 transition-colors">
                      <div className="bg-blue-50 text-blue-600 w-12 h-12 rounded-xl flex flex-col items-center justify-center shrink-0">
                        <span className="text-xs font-bold uppercase">{format(eventDate, "MMM")}</span>
                        <span className="text-lg font-black leading-none">{format(eventDate, "d")}</span>
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className="font-semibold truncate">{event.title}</h4>
                        <p className="text-sm text-slate-500 truncate">
                          {isToday ? "Today" : format(eventDate, "EEEE")} • {event.startTime}
                        </p>
                      </div>
                    </div>
                  )
              ...
              {recentNotes.length > 0 ? (
                recentNotes.map(note => {
                  ...
                  >
                    ...
                    <div className={`flex-1 text-sm font-medium whitespace-pre-wrap ${colorTheme.text} leading-relaxed mt-2 line-clamp-4`}>
                      {note.text}
                    </div>
